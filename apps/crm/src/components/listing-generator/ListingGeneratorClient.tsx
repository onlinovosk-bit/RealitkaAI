...
const PERSONAS: { value: ListingPersona; label: string; hint: string }[] = [
  ...
  { value: "INVESTOR", label: "Investor", hint: "Lokalita, dopyt, potenciál — bez sľubu výnosu" },
  { value: "DOWNSIZER", label: "Zmenšujúci bývanie (50+)", hint: "Nízka údržba, výťah, dostupnosť" },
];

type Channel = {
  key: "portal_text" | "fb_ad_copy" | "ig_caption" | "email_subject" | "email_body";
  label: string;
  emoji: string;
  multiline: boolean;
};
...
export default function ListingGeneratorClient() {
  ...
  const [charakterKind, setCharakterKind] = useState<CharakterLokalityKind | "">("");
  const [charakterText, setCharakterText] = useState("");
  ...
  function setField<K extends keyof PropertyInput>(k: K, v: PropertyInput[K]) {
    setProperty((p) => ({ ...p, [k]: v }));
  }

  function buildPayload(): PropertyInput {
    return sanitizePropertyInput({
      ...property,
      features: featuresRaw
        ...
        .filter(Boolean),
      charakterLokality: {
        ...(charakterKind ? { kind: charakterKind } : {}),
        ...(charakterText.trim() ? { text: charakterText.trim() } : {}),
      },
    });
  }
  ...
  function editField(key: Channel["key"], value: string) {
    ...
    <div className="space-y-6">
      <section className="rounded-xl border border-slate-800 bg-slate-900/50 p-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <label className={labelCls} htmlFor="lg-type">
              Typ nehnuteľnosti *
            </label>
            <input
              id="lg-type"
          ...
          <div>
            <label className={labelCls} htmlFor="lg-location">
              Lokalita *
            </label>
            <input
              id="lg-location"
          ...
          <div>
            <label className={labelCls} htmlFor="lg-district">
              Časť / štvrť
            </label>
            <input
              id="lg-district"
              className={inputCls}
              placeholder="Sídlisko III"
              value={property.district ?? ""}
              onChange={(e) => setField("district", e.target.value)}
            />
          </div>
          <div>
            <label className={labelCls} htmlFor="lg-charakter-kind">
              Charakter lokality (voliteľné)
            </label>
            <select
              id="lg-charakter-kind"
              className={inputCls}
              value={charakterKind}
              onChange={(e) =>
                setCharakterKind((e.target.value || "") as CharakterLokalityKind | "")
              }
            >
              <option value="">— bez povahy lokality —</option>
              {CHARAKTER_LOKALITY_KINDS.map((k) => (
          ...
          <div className="sm:col-span-2">
            <label className={labelCls} htmlFor="lg-charakter-text">
              Charakter lokality — voľný text (voliteľné)
            </label>
            <textarea
              id="lg-charakter-text"
              rows={2}
              className={inputCls}
              placeholder='Napr. „Sabinov nie je Prešov — pokojnejšie malé mesto." Bez vyplnenia AI nepíše o povahe lokality.'
              value={charakterText}
              onChange={(e) => setCharakterText(e.target.value)}
            />
          </div>
          <div>
            <label className={labelCls} htmlFor="lg-rooms">
              Dispozícia
            </label>
            <input
              id="lg-rooms"
          ...
          <div>
            <label className={labelCls} htmlFor="lg-size">
              Výmera (m²) *
            </label>
            <input
              id="lg-size"
              ...
              onChange={(e) => setField("size_m2", Number(e.target.value))}
            />
          </div>
          <div>
            <label className={labelCls} htmlFor="lg-price">
              Cena (€) *
            </label>
            <input
              id="lg-price"
          ...
          <div>
            <label className={labelCls} htmlFor="lg-floor">
              Poschodie
            </label>
            <input
              id="lg-floor"
          ...
          <div>
            <label className={labelCls} htmlFor="lg-condition">
              Stav
            </label>
            <select
              id="lg-condition"
          ...
          <div className="sm:col-span-2">
            <label className={labelCls} htmlFor="lg-features">
              Vybavenie (oddeľ čiarkou)
            </label>
            <input
              id="lg-features"
              className={inputCls}
              placeholder="balkón, parkovanie, pivnica"
              value={featuresRaw}
              onChange={(e) => setFeaturesRaw(e.target.value)}
            />
          </div>
          <div className="sm:col-span-2">
            <label className={labelCls} htmlFor="lg-notes">
              Poznámky makléra
            </label>
            <textarea
              id="lg-notes"
