...
export default function CalendarSyncPanel() {
  ...
  async function handleSync() {
    ...
    try {
      ...
      setStatus("done");
      setMessage("Kalendár bol úspešne synchronizovaný.");
    } catch (err) {
      setStatus("error");
      setMessage(err instanceof Error ? err.message : "Calendar sync zlyhal.");
    }
  }

  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
      <h2 className="text-lg font-semibold text-gray-900">Kalendár (ICS)</h2>
      <p className="mt-1 text-sm text-gray-500">
        Synchronizuj udalosti z ICS kalendára.
      </p>
      ...
      >
        {status === "syncing" ? "Synchronizujem..." : "Synchronizovať kalendár"}
      </button>
    </div>
  );
}
