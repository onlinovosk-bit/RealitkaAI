...
export default function GmailSyncPanel() {
  ...
  async function handleSync() {
    ...
    try {
      ...
      setStatus("done");
      setMessage("Emaily boli úspešne synchronizované.");
    } catch (err) {
      setStatus("error");
  ...
  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
      <h2 className="text-lg font-semibold text-gray-900">Email (IMAP)</h2>
      <p className="mt-1 text-sm text-gray-500">
        Synchronizuj nové emaily z IMAP schránky.
      </p>
      ...
      >
        {status === "syncing" ? "Synchronizujem..." : "Synchronizovať emaily"}
      </button>
    </div>
  );
}
