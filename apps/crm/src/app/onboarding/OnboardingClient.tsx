...
export default function OnboardingClient() {
  ...
  useEffect(() => {
    // 1. localStorage — okamžité, bez siete
    ...
    if (savedData) {
      ...
      } catch { /* poškodené dáta — ignoruj */ }
    }

    setLoading(false); // Nezablokuj UI na Supabase

    // 2. Supabase sync na pozadí — len ak existuje session
    if (savedId) {
      (async () => {
        try {
