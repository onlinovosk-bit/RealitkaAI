...
export const AGENT_SPECS: readonly AgentSpec[] = [
  ...
  },
  {
    ...
    version: "1.0.0",
    mission: "Write and send a personalised outreach e-mail when a broker asks for it.",
    trigger: "POST /api/outreach/{send,approve} (broker click); cron/script callers are refused",
    inputs: ["one lead (status in OUTREACH_ALLOWED_STATUSES)", "A/B variant"],
    outputs: ["e-mail via Resend", "ai_action_audit ai_suggested → sent | send_failed", "conversation + activity"],
    ...
    approvalPolicy: "Broker's click passes a human approval into sendAiOutreachEmail; without one the Control Contract refuses before generation.",
    memoryPolicy: "Reads last-send time for the frequency cooldown; no free-form memory.",
    failurePolicy: "Daily limit, per-lead cooldown and Resend errors throw with an audit row; nothing is sent.",
    evaluationSuite: [
      "src/lib/__tests__/outreach-contract.test.ts",
      "tests/verification/outreach-scoped-lead-lookup.verification.test.ts",
    ],
    owner: "founder",
    code: ["src/lib/outreach-store.ts", "src/lib/ai-outreach.ts"],
  },
];
