...
-- lead_property_events: PROD drift, no migration creates it, so guard the
-- statements on the table existing. `DROP POLICY IF EXISTS` tolerates a missing
-- policy but not a missing table — it raises 42P01, which is how CI rejected
-- the first version of 20260925110000.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_tables
    WHERE schemaname = 'public' AND tablename = 'lead_property_events'
  ) THEN
...
-- leads_demo: also PROD drift. Capture runs on the service role.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_tables
    WHERE schemaname = 'public' AND tablename = 'leads_demo'
  ) THEN
