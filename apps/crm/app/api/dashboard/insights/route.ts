...
import { getUserHistory, saveUserHistory } from "@/lib/ai-insights/history";
import { logAnalyticsEvent } from "@/lib/ai-insights/analytics";
  ...
  // TODO: zavolaj LLM prompt, zatiaľ mock
  let insights: DashboardInsightsResponse = {
    ...
    summary:
      `Najväčší potenciál vidíme v kupujúcich s rozpočtom 150–250k € v Prešove a Košiciach. Posledné 3 dni boli aktívni, ale nedostali žiadny personalizovaný follow‑up.`,
    actions: [
      {
        title: `Zavolaj top 3 hot leadom s readiness score > 90` + (userName ? ` (${userName})` : ""),
        description:
          `Títo klienti reagovali na posledný email, pozerali si nové ponuky a sú blízko rozhodnutiu. Navrhni im 2–3 konkrétne nehnuteľnosti a dohodni obhliadku.`,
      ...
      },
      {
        ...
        description:
          'Udržíš ich v hre a získaš lepší signál o záujme. AI ti navrhne text na mieru.',
    ...
    notesForOwner:
      'Najaktívnejší makléri generujú 80 % obratov. Zváž, či ostatným nepomôže zjednodušený denný plán založený na týchto akciách.',
  };
  ...
  // História (demo: načítaj a vráť späť, v reálnom FE sa bude volať saveUserHistory pri akcii)
  const userHistory = await getUserHistory(userId);
  // insights.userHistory = userHistory; // FE si môže pýtať samostatne
