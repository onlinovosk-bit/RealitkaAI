...
**Záver:** `activities` je UI log, nie event stream. Nie je kandidátom na spine ani na zdroj reaction events.
...
// ——— REPORT OUTCOME ———
export type Outcome =
  | { kind: "measured"; outcome: string; valueEur: number | null; measuredAt: string }
  | { kind: "unknown";  reason: "too_early" | "not_observable" | "source_missing"; recheckAfter: string | null };
...
| `risk = irreversible` | `FORBIDDEN` (bez ohľadu na confidence) |
| `externallyVisible = true` (e-mail, SMS klientovi) | `APPROVAL_REQUIRED` |
...
**Invariant:** `FORBIDDEN` nesmie byť prekonateľné approvalom. Approval mení `APPROVAL_REQUIRED → povolené`, nikdy `FORBIDDEN → povolené`.
...
>
> **MIGRATION IMPACT:** ADD COLUMN × 8 + indexy. **Žiadny backfill dát** — historické riadky dostanú `schema_version = 1` a `NULL` v nových poliach. Riadky v1 a v2 koexistujú natrvalo.
...
**Indexy [TARGET]:** `(agency_id, occurred_at DESC)` · `(correlation_id)` · `(entity_type, entity_id, occurred_at DESC)` · `UNIQUE (agency_id, event_type, idempotency_key) WHERE idempotency_key IS NOT NULL`.
...
| Konzument **musí** vetviť podľa `schema_version` alebo čítať cez kompatibilný view | v1 a v2 koexistujú natrvalo |
...
> **DECISION D-06:** `agency_id` prejde na `NOT NULL`, ale **až po** explicitnom GO, a v dvoch krokoch.
> **WHY:** Dnes je stĺpec nullable a RLS policy má vetvu `agency_id IS NULL OR ...` — riadok s NULL tenantom je **viditeľný pre každého prihláseného používateľa naprieč agentúrami**. Dnes je to latentné (0 takých riadkov), nie aktívne. Ale je to ten istý vzor ako AP-002.
> **ALTERNATIVES:** (a) Nechať nullable a len opraviť policy — zamietnuté, nechráni pred budúcim NULL insertom. (b) Okamžité NOT NULL — zamietnuté, je to zmena constraintu na živej tabuľke, patrí do vlastnej brány.
> **TRADE-OFFS:** Systémové eventy bez tenanta (napr. `system.cron_ran`) potrebujú miesto. Riešenie: vyhradené sentinel `SYSTEM_USAGE_AGENCY_ID` (už existuje v `lib/usage-metrics`), nie NULL.
> **CONSEQUENCES:** `emit_platform_event` musí odmietnuť NULL tenant.
> **REVERSIBILITY:** Stredná. **MIGRATION IMPACT:** krok 1 = oprava policy (odstrániť `IS NULL` vetvu) + guard; krok 2 = `SET NOT NULL`. Obe pod samostatným GO.
...
2. ADD COLUMN × 8 + indexy          aditívne, NULLABLE, schema_version DEFAULT 1
...
**Pripravené (bez blokérov):**
- `CP-P0-4` Control Contract — typy, authority model, approval kontrakt, akceptačné kritériá.
- `CP-P0-2` Durable Approvals — kontrakt hotový, dáta žiadne.
...
**Vykonané:** špecifikácia. **Nevykonané:** implementácia, migrácia, zmena schémy, oprava `lead_events`, UI, refaktor konzumentov, merge, deployment.
**STOP.** Ďalší krok je adversariálny architektonický review tohto dokumentu, nie kód.
