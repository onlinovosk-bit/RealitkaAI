...
## 1. Token (mimo repa, mimo chatu)

```bash
openssl rand -hex 32
export REVOLIS_BUS_TOKEN='<vygenerovaná hodnota>'
```
...
Riadok `store:` musí sedieť s tým, čo si zvolil v kroku 0.
...
4. Authentication → API Key → Auth Type **Bearer** → vlož `REVOLIS_BUS_TOKEN`
   (tu, do ChatGPT konfigurácie — nie do repa)
...
| `403 box_not_writable` | zápis do `archive`/`state` | cez HTTP sa píše len `inbox`/`tasks`/`context`/`decisions` |
